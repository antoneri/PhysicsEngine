...
{
	public abstract List<Intersection> Collides (PE.ParticleSystem ps);

	public static List<Intersection> Collides (PE.ParticleSystem ps, Plane pl)
	{
		List<Intersection> intersections = new List<Intersection> ();
		foreach (var p in ps) {
			double d = Math.Abs (Vec3.Dot (p.x, pl.normal) - pl.d);
			if (d < Plane.thickness) {
				var data = new Intersection {
					particle = p,
	...
	public static List<Intersection> Collides (PE.ParticleSystem ps, AABB b)
	{
		List<Intersection> intersections = new List<Intersection> ();
		foreach (var p in ps) {
...
public class Plane : Collider
{
	...
	public override List<Intersection> Collides (PE.ParticleSystem ps)
	{
